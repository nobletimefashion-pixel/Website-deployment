...
from nexus_agent.config.config import Config, HookConfig, HookTrigger
from nexus_agent.Tools.base import ToolResult
#1633
...
class HookSystem:
    ...
    def _build_env(
        ...
        env = os.environ.copy()
        env["NEXUS_AGENT_TRIGGER"] = trigger.value
        env["NEXUS_AGENT_CWD"] = str(self.config.cwd)

        if tool_name:
            env["NEXUS_AGENT_TOOL_NAME"] = tool_name

        if user_message:
            env["NEXUS_AGENT_USER_MESSAGE"] = user_message

        if error:
            env["NEXUS_AGENT_ERROR"] = str(error)
    ...
    async def trigger_after_agent(
        ...
        env["NEXUS_AGENT_RESPONSE"] = agent_response
    ...
    async def trigger_before_tool(
        ...
        env["NEXUS_AGENT_TOOL_PARAMS"] = json.dumps(tool_params)
    ...
    async def trigger_after_tool(
        ...
        env["NEXUS_AGENT_TOOL_PARAMS"] = json.dumps(tool_params)
        env["NEXUS_AGENT_TOOL_RESULT"] = tool_result.to_model_output()
