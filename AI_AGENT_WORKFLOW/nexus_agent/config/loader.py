import logging
from pathlib import Path
from typing import Any
import tomli
from platformdirs import user_config_dir, user_data_dir
from nexus_agent.config.config import Config
from nexus_agent.utils.errors import ConfigError
#7

CONFIG_FILE_NAME = 'config.toml'
AGENT_MD_FILE = "Agent.md"
logger = logging.getLogger(__name__)

#user configuration is staying
def get_config_dir() -> Path:
    return Path(user_config_dir('nexus-agent'))

def get_data_dir() -> Path:
    return Path(user_data_dir('nexus-agent'))
#system related cnofiguration is stying
def get_system_config_path() -> Path:
    return get_config_dir() / CONFIG_FILE_NAME

def _parse_toml(path: Path):
    try:
        with open(path, "rb") as f:
            return tomli.load(f)
    except tomli.TOMLDecodeError as e:
        raise ConfigError(f"Invalid TOML: {path}: {e}", config_file=str(path)) from e
    except (OSError, IOError) as e:
        raise ConfigError(f"failed to read config file: {path}: {e}", config_file=str(path)) from e


def _get_project_config(cwd: Path) ->Path | None:
    current = cwd.resolve()
    agent_dir = current / '.nexus-agent'
    
    if agent_dir.is_dir():
        config_file = agent_dir /CONFIG_FILE_NAME
        if config_file.is_file():
            return config_file
        
    return None

def _get_agent_md_file(cwd: Path) ->Path | None:
    current = cwd.resolve()
    
    if current.is_dir():
        agent_md_file = current / AGENT_MD_FILE
        if agent_md_file.is_file():
            content = agent_md_file.read_text(encoding="utf-8")
            return content
        
    return None

def _merge_dict(base: dict[str,Any], override: dict[str,Any]) -> dict[str,Any]:
    result = base.copy()
    for key,val in override.items():
        if key in result and isinstance(result[key], dict) and isinstance(val,dict):
            result[key] = _merge_dict(result[key], val)
        else:
            result[key] = val
    return result

def load_config(cwd: Path | None) -> Config:
    cwd = cwd or Path.cwd()
    # ~/.config/ai-agent/config.toml
    
    system_path = get_system_config_path()
    
    config_dict: dict[str,Any] = {}
    
    if system_path.is_file():
        try:
            config_dict = _parse_toml(system_path)
        except ConfigError:
            logger.warning(f"Skipping invalid config file: {system_path}")
            
    #users/dir/nexus-agent/.config.toml
    project_path = _get_project_config(cwd)
    if project_path:
        try:
            project_config_dict = _parse_toml(project_path)
            config_dict = _merge_dict(config_dict, project_config_dict)
        except ConfigError:
            logger.warning(f"Skipping invalid config file: {system_path}")
            
    if "cwd" not in config_dict:
        config_dict["cwd"] = cwd
        
    if "developer_instructions" not in config_dict:
        agent_md_content = _get_agent_md_file(cwd)
        if agent_md_content:
            config_dict["developer_instructions"] = agent_md_content
    try:
        config = Config(**config_dict)
    except Exception as e:
        raise ConfigError(f"invalide configuration: {e}") from e
    return config